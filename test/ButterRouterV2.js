...
describe("ButterRouterV2", function () {
    ...
    it("setFee only owner", async () => {
        let [wallet, other] = await ethers.getSigners();
        await deployFixture();
        await expect(router.connect(other).setFee(wallet.address, 100, 10000000)).to.be.revertedWith(
    ...
    it("setFee feeReceiver zero address", async () => {
        let [wallet, other] = await ethers.getSigners();
        await deployFixture();
        await expect(router.connect(wallet).setFee(ethers.constants.AddressZero, 1000, 100)).to.be.revertedWith(
            "ButterRouterV2: zero addr"
        );
    });
    ...
    it("swapAndCall", async () => {
        ...
        await (
            await router
                .connect(user)
                .swapAndCall(ethers.constants.HashZero, _srcToken, _amount, 0, _swapData, _callData, _permitData, {
    ...
    it("swapAndCall", async () => {
        ...
        await (
            await router
                .connect(user)
                .swapAndCall(ethers.constants.HashZero, _srcToken, _amount, 1, _swapData, _bridgeData, _permitData)
        ).wait();
        let balanceAfter = await tokenOut.balanceOf(user.address);
    ...
    it("swapAndBridge", async () => {
        ...
        let remote = ethers.utils.defaultAbiCoder.encode(
            ["tuple(uint256,uint256,bytes,uint64)[]", "bytes", "address"],
            [[], "0xb6a7276b87b9763898c38ea19b7573cd81e6af5643031b835d15aa2ad6000442", ethers.constants.AddressZero]
        );
    ...
    it("swapAndCall", async () => {
        ...
        await expect(
            router
                .connect(user)
                .swapAndCall(ethers.constants.HashZero, _srcToken, _amount, 1, _swapData, _payData, _permitData)
        )
            .to.be.emit(pay, "Pay")
    ...
    it("remoteSwapAndCall _makeUniV3Swap -> native in ", async () => {
        ...
        let path =
            "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc20001f4a0b86991c6218b36c1d19d4a2e9eb0ce3606eb480027106982508145454ce325ddbe47a25d4ec3d2311933";
        ...
        swap = ethers.utils.defaultAbiCoder.encode(
            ["tuple(uint8,address,address,address,address,uint256,bytes)"],
            [[0, swapAdapter.address, swapAdapter.address, user.address, dstToken, 0, data]]
        );
        ...
        await expect(
            mos.connect(user).mockRemoteSwapAndCall(router.address, _srcToken, amount, swapAndCall, { value: amount })
    ...
    it("remoteSwapAndCall _makeAggFill -> ", async () => {
        ...
        let swap = interface.encodeFunctionData("swapExactTokensForTokens", [
            0,
            amountOutMin,
            path,
            swapAdapter.address,
    ...
    it("remoteSwapAndCall _makeCurveSwap", async () => {
        ...
        let swap = ethers.utils.defaultAbiCoder.encode(
            ["uint256", "address[9]", "uint256[3][4]", "address[4]"],
            [amountOutMin, _route, _swap_params, pools]
        );
        let SwapData = {
            dexType: 3,
            callTo: curverouter,
    ...
    it("remoteSwapAndCall buy nft seaport", async () => {
        ...
        let call =
            "";
        let _payData = ethers.utils.defaultAbiCoder.encode(
            ["tuple(address,address,uint256,uint256,address,bytes)"],
            [[seaport, seaport, amount, 0, user.address, call]]
        );
